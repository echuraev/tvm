...
def test_split_concat(target_name, shape_type):
    ...
    def expected(limit):
        ...
        for inp in inputs:
            ...
            if added_args + curr_args > limit:
                ...
                last_op = relay.annotation.stop_fusion(concat)
                new_args = [last_op]
                added_args = 1
    ...
    res = run_opt_pass(
        before(),
        transform.SplitArgs(tvm.target.Target(target_name).max_function_args),
    )
    exp = run_opt_pass(
        expected(tvm.target.Target(target_name).max_function_args), transform.InferType()
    )
