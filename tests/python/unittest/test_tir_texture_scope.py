...
def test_texture_array_nhwc_scope():
    @tvm.script.ir_module
    class PlusOneMultTwo:
        @T.prim_func
        def main(a: T.handle, b: T.handle) -> None:
            T.func_attr({"global_symbol": "main", "tir.noalias": True})
            A = T.match_buffer(a, (128, 128, 4), dtype="float32", scope="global.texture-array-nhwc")
            B = T.alloc_buffer((128, 128, 4), dtype="float32", scope="global.texture-array-nhwc")
            C = T.match_buffer(b, (128, 128, 4), dtype="float32", scope="global.texture-array-nhwc")
            for block_idx in T.thread_binding(0, 128, thread="blockIdx.x"):
                for thread_idx in T.thread_binding(0, 128, thread="threadIdx.x"):
                    for k in T.serial(4):
                        with T.block("B"):
