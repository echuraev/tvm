...
def compute_conv2d_NHWC_HWIO(Input, Filter, stride, padding, dilation, out_dtype=None, args={}):
    ...
    if convert_from4d and autotvm.GLOBAL_SCOPE.in_tuning == False:
      dummy_cast = te.compute((batch, out_height_orig, out_width_orig, out_channel_chunks, out_channel_block),
                               lambda n, y, x, fc, fb: conv[n,y,x,fc, fb].astype(out_dtype), tag="dummy_cast")
      return te.compute((batch, out_height_orig, out_width_orig, out_channles),
                        lambda n, y, x, c: dummy_cast[n,y, x, c // out_channel_block, c % out_channel_block],
                        tag="cast_from_acc" + args["accumulator"][-2:])
    else:
      return te.compute((batch, out_height_orig, out_width_orig, out_channel_chunks, out_channel_block),
                         lambda n,y,x,ffc, ffb: conv[n,y,x,ffc,ffb].astype(out_dtype), tag="cast_from_acc" + args["accumulator"][-2:])
