...
def compute_conv2d_NCHWc_KCRSk(Input, Filter, stride, padding, dilation, out_dtype=None, args={}):
    ...
    if convert_from4d and autotvm.GLOBAL_SCOPE.in_tuning == False:
      dummy_cast = te.compute((batch, out_channel_chunks, out_height_orig, out_width_orig, out_channel_block),
                               lambda n,fc,y,x,fb: conv[n,fc,y,x,fb].astype(out_dtype), tag="dummy_cast")
...
def schedule_conv2d_NCHWc_KCRSk(cfg, s, output, args={}):
    ...
    #TODO(amalyshe) add testcase
    #TODO(amalyshe) move this part to utils, reuse everywhere
    if conv.shape[1] % 2 == 0:
        min_threads_div = 2
    else:
        min_threads_div = 1
    ...
    cfg.define_split("tile_fc", fc, num_outputs=3,
                filter=lambda entity: entity.size[1] <= 8 and entity.size[2] >= min_threads_div and entity.size[2] < 256 )
    ...
    # TODO(amalyshe): move multi_filter functionality and uncomment
