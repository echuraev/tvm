...
from .utils import split_to_chunks, pack_input, pack_filter, expand_spatial_dimensions, add_pad, bind_data_copy
...
def compute_depthwise_conv2d_NCHWc_KCRSk(Input, Filter, stride, padding, dilation, out_dtype=None, args={}):
    ...
    out_height_orig, out_height, out_width_orig, out_width = expand_spatial_dimensions(in_height, in_width,
                              kernel_h, kernel_w,
    ...
    conv = te.compute(
        ...
        lambda nn, ffc, yy, xx, ffb: te.sum(
            (temp[nn, ffc//in_filter_channels, yy * stride_h + ry * dilation_h, xx * stride_w + rx * dilation_w, ffb]
            * Filter[ffc//in_filter_channels, ffc % in_filter_channels, ry, rx, ffb]).astype(args["accumulator"]),
            axis=[ry, rx],
        ),
    ...
    if convert_from4d and autotvm.GLOBAL_SCOPE.in_tuning == False:
      dummy_cast = te.compute((batch, out_channel_chunks, out_height_orig, out_width_orig, out_channel_block), lambda n,fc,y,x,fb: conv[n,fc,y,x,fb].astype(out_dtype), tag="dummy_cast")
      return te.compute((batch, out_channles, out_height_orig, out_width_orig), 
                lambda n,c,y,x: dummy_cast[n,c // out_channel_block, y, x, c % out_channel_block],
                tag="cast_from_acc" + args["accumulator"][-2:])
    else:
      return te.compute((batch, out_channel_chunks, out_height_orig, out_width_orig, out_channel_block),
                         lambda n,ffc,y,x,ffb: conv[n,ffc,y,x,ffb].astype(out_dtype), tag="cast_from_acc" + args["accumulator"][-2:])
...
def schedule_depthwise_conv2d_NCHWc_KCRSk(cfg, s, output, args={}):
    ...
    if isinstance(kernel.op, tvm.te.ComputeOp) and "filter_pack" in kernel.op.tag: #len(latest.op.axis) == 4:
    ...
    # create cache stage
    #TODO(amalyshe): figure out if we need to call cache_read for tuning
    #without cache read on sample depthwise conv we get 18GFLOPS, 1.2ms in tune and 4.0ms in inference
    #with cahce read we have 10GFLOPS, 1.2ms in tune and 4.0ms in inference
    #why do we see different number of GFLOPS???
    if autotvm.GLOBAL_SCOPE.in_tuning or len(latest.op.axis) == 4:
        ...
        # TODO(amalyshe): this branch should go away after enabling of textures static planning
        # it is left by intend of leaving correct condition above
        AT = s.cache_read(pad_data, "global.texture", [conv])
        WT = s.cache_read(kernel, "global.texture-weight", [conv])
        bind_data_copy(s[AT])
        bind_data_copy(s[WT])
