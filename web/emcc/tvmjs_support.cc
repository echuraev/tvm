...
class AsyncLocalSession : public LocalSession {
 ...
 private:
  ...
  PackedFunc WrapWasmTimeEvaluator(PackedFunc pf, Device dev, int number, int repeat,
    ...
    auto ftimer = [pf, dev, number, repeat, min_repeat_ms, cooldown_interval_ms](TVMArgs args,
                                                                                 TVMRetValue* rv) {
      ...
      auto finvoke = [pf, values, type_codes]() {
        TVMRetValue temp;
      ...
      (*time_exec)(TypedPackedFunc<void()>(finvoke), dev, number, repeat, min_repeat_ms,
                   cooldown_interval_ms, on_complete);
    };
